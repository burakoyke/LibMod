...
public class DependencyMatrixGenerator_2 {
    public static void main(String[] args) throws IOException {
        String inputFilePath = "packages_packages_dependencies.rsf";
        String outputFilePath = "dependency_matrix_2.txt";
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath))) {
            List<String> packages = new ArrayList<>(packageIndexMap.keySet());
            Collections.sort(packages, Comparator.comparingInt(packageIndexMap::get));

            for (int i = 0; i < size; i++) {
                writer.write((i + 1) + " " + packages.get(i) + " ");
